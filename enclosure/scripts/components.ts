...
// だけを見ていたが、当時の body のラグも同じ「静かに分離する」失敗モードを持っていたので
// 汎用にしてループで全パートを見る。ラグはその後プレート外形へ統合されて消えたが、
// カバー側の耳ウェブ／ガセットは同じ失敗モードを持ち続ける）。
//
...
// 基準としては脆く、ここでは使わない（既知の値: 現行カバー単体で Genus=79。内訳は
// 概念上の貫通穴 83 個 [M2 耳4 + スイッチ穴1 + USB切欠き1 + 屋根ハニカム77] のうち
// M2 耳4個が上記の縮退で種数0にカウントされるため 83-4=79。屋根のハニカムを 1 個
// 増減させるだけで動く数字なので、なおさら判定基準には使えない）。
