...
// pcb_tall_y_max=78.4 のずっと内側。下記 sw_pt 周辺のコメント参照）が独立に存在する。
...
// 開口: LED 窓（屋根の斜面。素通し穴）。y=79.8 での屋根内面 z = roof_in_z(79.8) = 28.0
// （基板上面 11.4 から 16.6mm）。
// LED（φ5 × 8.6）は窓の真下に座り、窓越しに真上から覗く格好になる。頭を窓まで持ち上げる
// ことはできない: 屋根が 45° で降りているので先に当たるのは LED の +Y リム（y = 82.3、
// そこの屋根内面は 25.5）で、足を伸ばせる幾何上限は 25.5 - 11.4 - 8.6 = 5.5mm。
// pcb_stack_mid = 12.0 の予算内に収めるなら 3.4mm まで。どちらにしても頭は窓の内口
// （斜面上の穴の縁。x=-15 断面では y 76.3〜80.5）より下に留まる（設計書の Ruling 13）。
// なお窓は φ6 なので +Y リムが y 82.8 まで来て基板の +Y 端（82.3）を 0.5mm はみ出す。
// 屋根に開いた穴なので当たる実体は無く、窓の縁からトレイ床がわずかに覗くだけ。
// cover_test.scad の「LED 窓が基板の Y 範囲」assert を中心で見ているのはこのため。
...
sw_face_z = cover_top_z - (sw_pt[1] - cover_slope_y0);   // 55.63
sw_tip_z  = sw_face_z - sw_depth*cos(45);                // 37.24
// 基板側 keep-out: 本体を φsw_body_d の円柱として法線方向に sw_depth 掃引すると、
// ワールド x -6.8〜4.8 / y 32.5〜59.1 を通り、最下点は (x -1, y 40.7, z 33.18)。
// x/y の帯は sw_pt・sw_depth・45° 勾配だけで決まり cover_slope_y0 に依存しないが、
// z（33.18）は cover_slope_y0 のぶんだけ底上げされている（cover_slope_y0 を手置きの
// 定数にした Ruling 10 / 値を 34.9 にした Ruling 12 に対応。以前の値からのズレは
// そのつど cover_slope_y0 の変化ぶんだけ連動する）。
// 軸上の先端 z（37.24）ではなく、この最下点が基板の部品と当たるかを決める。
// この帯には pcb_stack_tall 級（上端 27.4）の部品を置かないこと。Pico スタック
// （上端 20.9）なら 12.28mm の余裕がある。cover_test.scad の assert が下限を守る。
