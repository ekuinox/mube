...
// ─── ホーン嵌合インターフェース（socket / drive-gear 共用）────────────────────
...
// difference 側: バー+ハブポケット彫り込み & 爪逃がし溝
// ローカル座標: ポケット底 z=0、バー面 -Z 側（socket_test / drive-gear 共通）
module horn_pocket_cuts() {
  hc = horn_clearance;
  translate([0, 0, -0.1])
...
// union 側: 中心スタブ & 押さえ爪 4 本
module horn_pocket_adds() {
  hc = horn_clearance;
