...
assert(sw_pt[0] - sw_body_d/2 >= servo_shaft_offset + servo_tab_l/2,
       "スイッチ本体がサーボの耳に当たる");
// 柱の +Y 最外点がトレイ床の -Y 端より外にあること（中心 sw_pt[1] ではなく半径ぶん
// +Y に張り出した点で見る）。座面の assert は sw_pt[1] を 25.4 まで許すが、そこだと
// 柱が y 31.15 まで届いてトレイ／基板の真上に入る。
// 支配関係の注記: 現在値では z 側にも大差（柱の底面 49.73 に対しトレイの最高点は固定
// スリーブ天面 12.2、基板の最高部品でも 27.4）があるので、xy が重なっても実体としては
// 当たらない。この assert は物理干渉ではなく「スイッチの柱を基板エリアの真上に置かない」
// という配置の不変条件を守るもの。柱の真下に手を入れる前提の配線・組立順（設計書の
// 組立手順 6-7）がこれに依存している。
assert(sw_pt[1] + sw_body_d/2 <= tray_y0, "スイッチ本体の柱がトレイ／基板の真上に入る");
