...
- ルート Cargo.toml: members = ["crates/*"] + default-members = ["crates/firmware", "crates/mube-core"]。cargo は 1 起動 1 ターゲットなので、wasm32 専用の webui は素の cargo build（thumbv6m 既定）から外し、trunk が --target wasm32-unknown-unknown を明示してビルドする。--workspace 付き実行は webui が thumbv6m に乗るため設計上失敗する（Cargo.toml にコメントあり）。
- webui の空 [workspace] と exclude を撤去。メンバ化により worktree でも root workspace が正しく解決され、trunk build --release が通ることを確認済み。
- webui のプロファイルはルートの [profile.wasm-release]（inherits="release", opt-level="z", debug=false）に移設し、Trunk.toml の cargo_profile で使用。[profile.release.package.webui] 方式は依存（yew/gloo 等）に opt-level=z が効かず wasm が約 9% 太ることがレビューで判明したため custom profile にした。
- webui/Cargo.lock を削除しルートの Cargo.lock に統合（既存ピンのバージョン変更なし、wasm-bindgen =0.2.121 維持）。
- 実測: wasm 131,729 bytes（統合前 131,756 / package override 方式 143,137）。firmware release の flash 使用量 554,076 bytes / 2MB。
