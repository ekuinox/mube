...
# Re-execs inside the Nix dev shell so openscad/python3/cloudflared are on PATH.
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
if ! command -v cloudflared >/dev/null 2>&1 || ! command -v openscad >/dev/null 2>&1; then
...
./build.sh
openscad -D 'part="assembly"' -o build/assembly.stl scad/smartlock.scad
