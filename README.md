...
## 回路（ネットリスト as code）
    ...
    nix develop -c ./circuit/netlist.py   # = uv run --script circuit/netlist.py
...
### 準備
    ...
    #   詳細は crates/firmware/cyw43-firmware/README.md
    # crates/firmware/src/config.rs の WIFI_SSID / WIFI_PASSWORD を実値に書き換える

### ビルド
    nix develop -c cargo build --locked           # thumbv6m-none-eabi（.cargo/config.toml で既定ターゲット指定済み）
