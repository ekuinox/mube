...
（サーボ SIG=GP16 / ゲート=GP17 / LED 赤=GP2・緑=GP3 / ボタン=GP5）と一致させている。ERC は静的チェックで生成物は無い（旧手配線表・部品表は廃止）。テストは `./test/erc.sh`（bun test）。
...
ロック状態は外付けの二色LED（D1）で表示する（施錠=赤 GP2 / 解錠=黄緑 GP3、コモンカソード）。
オンボード LED（CYW43）は TCP 接続状態の表示で、役割を分担する。
GP5 のタクトスイッチを押すと施錠⇄解錠をトグルできる（室内側の手動操作）。ボタンは
