...
（サーボ SIG=GP16 / ゲート=GP17 / LED 赤=GP2・緑=GP5 / ボタン=GP3）と一致させている。ERC は静的チェックで生成物は無い（旧手配線表・部品表は廃止）。テストは `./test/erc.sh`（bun test）。
...
実装用の**ユニバーサル基板配線図**も同じビューアで見られる（プルダウン「ユニバーサル基板 (実装用)」）。
実基板は横 A〜M・縦 1〜25 の格子（`board.ts` の `BOARD` = 13×25）で、**四隅の穴は固定用のため使用不可**
（図では×印）。配線図は `circuit/perfboard/` が生成し、部品の手置きは `circuit/perfboard/layout.ts` を
編集して調整する（`at: [x, y]` は左上原点・各部品の 1 番ピンの穴）。Pico は実測 GP0=E4=(4,3) で固定配置される。
...
ロック状態は外付けの二色LED（D1）で表示する（施錠=赤 GP2 / 解錠=黄緑 GP5、コモンカソード）。
オンボード LED（CYW43）は TCP 接続状態の表示で、役割を分担する。
GP3 のタクトスイッチを押すと施錠⇄解錠をトグルできる（室内側の手動操作）。ボタンは
...
（オンボード LED がハートビート）。サーボ給電は動作時だけ ON（GP17 の電源ゲート）。
