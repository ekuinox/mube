...
- [ ] **Step 6: タブ移動後もテストハーネスとレンダーが通ることを確認**

Run:
```bash
./test/render.sh test/servo_mount_test.scad && ./test/render.sh scad/body.scad
```
Expected: 両方 `OK: ...`。WARNING/ERROR なし。
（この時点では body はまだ `servo_mounts()` を呼ばず、サーボ持ち上げも未実施。タブ逃げ位置変更が単体でレンダー破綻しないことの確認。）
...
- [ ] **Step 3: body 単体をレンダーして成功を確認**

Run:
```bash
./test/render.sh scad/body.scad
```
Expected: `OK: /tmp/body.stl`、`Simple: yes`（manifold）、WARNING/ERROR なし。

- [ ] **Step 4: アセンブリ全体をレンダーして成功を確認**

Run:
```bash
./test/render.sh scad/smartlock.scad
```
Expected: `OK: /tmp/smartlock.stl`、`Simple: yes`、WARNING/ERROR なし。

- [ ] **Step 5: 目視確認（STL を生成し所見を記録）**

Run:
```bash
./test/render.sh scad/body.scad /tmp/body_servo.stl
```
