...
2MB (0x200000) を以下に分割する（数値は実装時にリンカスクリプトで確定。
境界はすべて 4KB セクタ境界に揃える）。

```
0x10000000  BOOT2 + ブートローダー   32KB   (embassy-boot-rp、めったに更新しない)
0x10008000  ACTIVE スロット         988KB   (実行中ファーム。XIP でここから実行)
0x100FF000  DFU スロット            992KB   (受信バッファ。ACTIVE + 1 セクタ)
0x101F7000  余白（将来用）           32KB
0x101FF000  STATE パーティション      4KB   (ブートローダー状態: swap/revert フラグ)
```

- アプリの `memory.x` は FLASH 原点を ACTIVE 先頭（0x10008000）へ変更する。
  boot2 セクションはブートローダー側だけが持つ。
- ブートローダーは新規クレート `crates/mube-boot`（ワークスペースに追加）。
- **サイズ収支が最初の関門**: 現行ファーム（CYW43 ブロブ約 230KB + WebUI wasm 込み）が
  ACTIVE 988KB に収まることをビルド実測で確認してから着工する。
  収まらない・将来の余裕が欲しい場合の逃げ道として、CYW43 ブロブ（ほぼ更新不要）を
  固定アドレスの共有領域に置き、両スロットから参照する構成に変更できる
  （その場合レイアウトを引き直す）。
