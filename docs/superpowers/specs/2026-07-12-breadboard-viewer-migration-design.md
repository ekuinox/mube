...
- 対応 issue: #70（この移植では**閉じない**。方針転換全体の追跡 issue として残す）
...
- パーク先: `feat/perfboard-wiring-diagram` tip `507f7e9` ／ クローズ済み PR #69。タグは打たない。GitHub はクローズした PR の head ref（`refs/pull/69/head`）を恒久保持するため、ブランチを消してもこの tip は PR #69 経由で辿れる＝別途タグでピン留めする必要がない。将来の**片面ユニバーサル基板**作業はここを復活点にする。
...
- PR を出す（本文で #70 を参照するが自動クローズはしない。#70 は方針転換の追跡用に開いたまま）。
