...
- **前提整備（本作業に含める）**:
  - embassy クレート群を動く単一 `rev` に固定し、`branch = "main"` 追従で壊れているビルドを復旧する。
- **作らないもの（非目標）**:
...
- embassy を `branch = "main"` のまま放置すると再び壊れる。再現性のため `rev` 固定＋`Cargo.lock` コミットに倒す。
...
## 4. 前提整備: embassy の rev 固定

- 現状 `branch = "main"` 追従のため feature 名ドリフトでビルド不能（例: `arch-cortex-m` は現行 `platform-cortex-m`）。
- 手順:
  1. embassy 全クレートを単一 `rev = "<commit>"` に固定（rev は embassy `examples/rp/Cargo.toml` が指すものに合わせ、example と API を一致させる）。
  2. `cargo build` のエラーに従い feature 名を現行 main へ総当たり修正。
  3. `cargo build` 緑を確認し `Cargo.lock` をコミット。
- コミット順: 「embassy pin」→「servo モジュール」→「main デモ配線」。
...
- **embassy rev 選定**: example と一致する rev でないと、`PioSpi::new` / `control.join` など WiFi 土台側の API も食い違う恐れ。WiFi 土台が引き続きビルドできることを必ず確認する。
