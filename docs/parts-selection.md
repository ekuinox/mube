...
- ① 1台分の理論按分合計: ¥3,599（按分額列の総和。理論上 1 台が消費する分だけの価値）
- ② 実際に 1 台作るために払う総額: ¥4,750（パック品も丸ごと買う前提の実支出。送料別）
  - 内訳: 秋月 ¥4,750（全点秋月、他店送りなし）
...
| 秋月電子通商 | [P-04583](https://akizukidenshi.com/catalog/g/g104583/) | パネル用押しボタンスイッチ モーメンタリー 丸型 白 PS21B-1 |
...
第一候補の IRLZ44N は秋月取扱なしのため、採用品は IRLB3813PBF（秋月 I-06270, N-ch 30V/260A）。Q1 はサーボの GND リターン（SERVO_RTN）を切るローサイドスイッチで、ゲートは Pico W の GP20（3.3V）から Rg(220Ω) 経由で駆動し Rgs(10k) でプルダウン、給電時のみ ON のワンショット（約 500ms）。
...
- ユニバーサル基板 eleshop UP-203GSR（[MA7128](https://eleshop.jp/shop/g/gMA7128/) ¥363, 両面, 既製四隅穴ピッチ 64×39）: 一度こちらへ変更したが（#68）、未使用のまま P-03229 に戻した。両面スルーホールが欲しい場合の代替。
- ユニバーサル基板 72×47.5mm（[P-00517](https://akizukidenshi.com/catalog/g/g100517/) ¥100）: 部品点数が増えた場合の代替。

### 注意点

- 二色LED（D1, OSRGHC5B32A）はコモンカソード。3 本足のうち共通カソード（K）を GND、赤アノード（R）を GP9、黄緑アノード（G）を GP10 へ。足の並びはデータシートで確認すること。赤・黄緑とも Vf=2.1V のため抵抗は両側 330Ω で明るさが揃う。施錠=赤・解錠=黄緑で点灯し、同時点灯はしない。
...
- ユニバーサル基板（P-03229）の既製の四隅マウント穴は φ3.2、中心間ピッチ 長辺66mm×短辺41mm。トレイはこの四隅穴に合わせた支柱 4 本で基板を受ける（M2 セルフタップ。φ3.2 に対しネジ山は効かず頭で押さえる）。
