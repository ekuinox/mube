...
- 電圧降下は無視できる: 3.3V は Rds(on) 規定点(4.5V)をわずかに下回るのみ。保守的に Rds(on) を Vgs=4.5V 時(2.6mΩ)の約 20 倍＝高々 50mΩ と見積もっても、SG90 の起動・失速ピーク（〜1A 程度）での降下は最悪約 50mV、実際は 10mV 前後で、5V 供給に対し無視できる。
