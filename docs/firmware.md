...
依存は crates.io 公開バージョンに固定し、Cargo.lock をコミットしているので `--locked` で完全に再現できる。

## ロジックの host テスト（実機不要）

    cargo host-test

ロックコマンド（LOCK/UNLOCK/STATUS）の解釈と状態機械、TCP serve ループ（行分割、接続ライフサイクル、エラー処理、長すぎ行の棄却）をモックで通しテストする。
`cargo host-test` は devShell が提供する別名で、実体は `cargo test -p smtlk-core --target <ホストトリプル>`（devShell の外ではこちらを直接叩く）。
...
WiFi 接続後、TCP ポート 6000 で 1 接続ずつコマンドを受け付ける。
1 行 1 コマンド（`\n` 区切り）で、接続中はオンボード LED が点灯する。
...
ボタンは Pico W の内部プルアップを使い（外付けプルアップ抵抗は付けない）、ボタン操作も TCP STATUS に反映される。
...
実機合わせはキャリブ定数だけを調整する。
角度→パルス変換の 4 定数（SERVO_MIN_US / SERVO_MAX_US / LOCK_DEG / UNLOCK_DEG）は `crates/smtlk-core/src/servo_math.rs` に集約し、整定待ち SETTLE_MS は `crates/firmware/src/servo.rs` にある。
