...
オプション（`services.mube-door-lock.*`）: `hostname` / `tunnelId` / `picoOrigin` /
`proxyPort` / `protocol` / `credentialsFile`。既定値は実環境の値なので通常は `enable` だけでよい。
...
- **cloudflared が繋がらない（error 1033 / 接続数 0）**: QUIC が塞がれた環境では
  `protocol: http2` が必須。モジュール既定値は http2。
